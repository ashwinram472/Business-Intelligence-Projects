...
def make_plot(counts):
    ...
    for val in counts:
        for type_of_word in val:
            if type_of_word[0] == "positive":
                positive_count.append(type_of_word[1])
            else:
                negative_count.append(type_of_word[1])
                
    plt.axis([-1, len(positive_count), 0, max(max(positive_count), max(negative_count))+200])
...
def stream(ssc, pwords, nwords, duration):
    ...
    def calc_total_running(newValues, total):
        if total is None:
            total = 0
    ...
    total = words.updateStateByKey(calc_total_running)
    total.pprint()
