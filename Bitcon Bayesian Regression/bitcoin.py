...
def computeDelta(wt, X, Xi):
    ...
    # YOUR CODE GOES HERE
    N = len(X)-1
    meanX, stdX,p,n,d = np.mean(X[:-1]), np.std(X[:-1]), 0.0, 0.0, 0.0
    
    
    for row , temp in Xi.iterrows():
        yi, stdXi , meanXi , val = temp[-1], np.std(temp[:-1]), np.mean(temp[:-1]), 0.0
        for i in range(0,N):
            val += (X[i] - meanX) * (temp[i] - meanXi)
        sim = val/(N * stdX * stdXi)
        fac = math.exp(wt * sim)
        d += fac
        n += yi*fac
    p = n/d
...
# YOUR CODE HERE
ols = smf.ols(formula = 'deltaP ~ deltaP90 + deltaP180 + deltaP360' , data = trainData)
model = ols.fit()
